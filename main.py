...
class Dino:
    ...
    def __init__(self):
        ...
        self.load_sprites()  # Load the sprite sheet and extract frames
        self.show()

    def load_sprites(self):
        path = os.path.join('assets/images/dino3.png')  # Path to your sprite sheet
        sprite_sheet = pygame.image.load(path).convert_alpha()

        # Assuming the sprite sheet has 3 frames horizontally for walking and 2 for jumping
        for i in range(3):
    ...
    def update(self, loops):
        if self.jumping:
            ...
            if self.y <= self.jump_stop:
                self.fall()
            self.current_frame = 1  # Use the jump frame
        elif self.falling:
            self.y += self.gravity * self.dy
            if self.y >= self.fall_stop:
                self.stop()
            self.current_frame = 2  # Use the fall frame
        elif self.onground and loops % 4 == 0:
            self.current_frame = (self.current_frame + 1) % 3  # Cycle through walk frames
